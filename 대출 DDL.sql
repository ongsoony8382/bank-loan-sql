...
ALTER TABLE TB_LOAN_APLY
CHANGE RECEIVE_DT STS_CHG_DT DATETIME NULL COMMENT '상태변경일시';
...
ADD COLUMN CONSEC_AUTO_PYMT_MM INT NOT NULL DEFAULT 0 
COMMENT '연속 자동이체 성공 개월 수(조건 미충족 시 0으로 초기화)';
...
-- 기준 금리 이력 테이블
CREATE TABLE TB_LOAN_BASE_RATE_HIST (
BASE_RATE_HIST_ID  BIGINT        NOT NULL COMMENT '기준 금리 이력 ID',
BASE_RATE_TP_CD    VARCHAR(10)   Ndb_odd_adv_4OT NULL COMMENT '기준 금리 유형 코드',
BASE_RATE          DECIMAL(5,3)  NOT NULL COMMENT '기준 금리값(%)',
APLY_DT            DATETIME      NOT NULL COMMENT '적용 일시',
...
CONSTRAINT PK_TB_BASE_RATE_HIST PRIMARY KEY (BASE_RATE_HIST_ID),
CONSTRAINT UQ_TB_BASE_RATE_HIST UNIQUE (BASE_RATE_TP_CD, APLY_DT)
);

ALTER TABLE TB_LOAN_BASE_RATE_HIST
MODIFY COLUMN APLY_DT DATE NOT NULL;


-- 가산 금리 정책 테이블 
CREATE TABLE TB_LOAN_ADD_INTR_RT_RULE (
LOAN_TP_CD   VARCHAR(10)   NOT NULL COMMENT '대출 유형 코드',
CRDT_GRD_CD  VARCHAR(2)    NOT NULL COMMENT '신용등급 코드',
ADD_INTR_RT  DECIMAL(5,3)  NOT NULL COMMENT '가산금리(%)',
...
CONSTRAINT PK_TB_ADD_INTR_RT_RULE PRIMARY KEY (LOAN_TP_CD, CRDT_GRD_CD)
);

-- 우대 금리 정책 테이블 
CREATE TABLE TB_LOAN_PREF_INTR_RT_RULE (
RULE_ID       INT          NOT NULL COMMENT '우대금리 정책 ID(항상 1)',
PREF_INTR_RT  DECIMAL(5,3) NOT NULL COMMENT '적용우대금리(%) 무조건 -0.200',
...
CONSTRAINT PK_TB_PREF_INTR_RT_RULE PRIMARY KEY (RULE_ID)
);

RENAME TABLE TB_BASE_RATE_HIST TO TB_LOAN_BASE_RATE_HIST;
RENAME TABLE TB_ADD_INTR_RT_RULE TO TB_LOAN_ADD_INTR_RT_RULE;
RENAME TABLE TB_PREF_INTR_RT_RULE TO TB_LOAN_PREF_INTR_RT_RULE;


ALTER TABLE TB_LOAN_PREF_INTR_RT_RULE
ADD REMK VARCHAR(100) NULL COMMENT '비고(설명)';

ALTER TABLE TB_LOAN_PREF_INTR_RT_RULE
ADD PREF_COND_NM VARCHAR(50) NOT NULL COMMENT '우대 조건명';
